...
static av_cold int mediacodec_init(AVCodecContext *avctx)
{
    ...
    // MediaFormat KEY_I_FRAME_INTERVAL semantics (Android docs):
    //   > 0 : maximum interval (seconds) between two I-frames
    //   == 0: every frame is a sync frame (all-keyframe stream)
    //   < 0 : only the first frame is a sync frame, no subsequent IDRs
    // avctx->gop_size is in frames, so convert via fps.
    if (avctx->gop_size > 0) {
        gop = (float)avctx->gop_size / (float)s->fps;
    } else if (avctx->gop_size == 0) {
        gop = 0.0f;     /* all-keyframe */
    } else {
        gop = -1.0f;    /* disable subsequent IDRs */
    }

    ff_AMediaFormat_setInt32(format, "frame-rate", s->fps);

    // API 25+ (Android 7.1+) supports a float KEY_I_FRAME_INTERVAL (sub-second
    // precision); older releases require int32 seconds. On the int32 path we
    // round to the nearest second and clamp short positive GOPs (< 0.5s) up to
    ...
    // all-keyframe stream.
    {
        ...
        if (sdk_int >= 25) {
            av_log(avctx, AV_LOG_DEBUG,
                   "Set i-frame-interval to %.3fs (float, API %d)\n",
                   gop, sdk_int);
            ff_AMediaFormat_setFloat(format, "i-frame-interval", gop);
        } else {
            int gop_int;
            if (gop >= 0.0f)
                gop_int = (int)(gop + 0.5f);  /* round half up */
            else
                gop_int = -1;
            if (avctx->gop_size > 0 && gop_int == 0) {
                gop_int = 1;
                av_log(avctx, AV_LOG_WARNING,
                       "API %d (<25) doesn't support fractional "
                       "i-frame-interval, coerce gop_size=%d (%.3fs) to 1s; "
                       "set gop_size >= fps for accurate behavior\n",
                       sdk_int, avctx->gop_size, gop);
            }
            av_log(avctx, AV_LOG_DEBUG,
                   "Set i-frame-interval to %ds (int, API %d)\n",
                   gop_int, sdk_int);
            ff_AMediaFormat_setInt32(format, "i-frame-interval", gop_int);
        }
    }
