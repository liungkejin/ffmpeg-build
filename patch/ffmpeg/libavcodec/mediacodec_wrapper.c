...
static const struct FFJniField jni_amediacodec_mapping[] = {
    ...
    { "android/os/Bundle", NULL, NULL, FF_JNI_CLASS, OFFSET(bundle_class), 0 },
        { "android/os/Bundle", "<init>", "()V", FF_JNI_METHOD, OFFSET(bundle_init_id), 0 },
        { "android/os/Bundle", "putInt", "(Ljava/lang/String;I)V", FF_JNI_METHOD, OFFSET(bundle_put_int_id), 0 },
...
static int mediacodec_jni_requestKeyFrame(FFAMediaCodec *ctx)
{
    ...
    if (!codec->jfields.set_parameters_id ||
        !codec->jfields.bundle_class ||
        !codec->jfields.bundle_init_id ||
        !codec->jfields.bundle_put_int_id) {
        return AVERROR(ENOSYS);
    }
    ...
    key = ff_jni_utf_chars_to_jstring(env, "request-sync", codec);
    if (!key) {
        ret = AVERROR_EXTERNAL;
        goto done;
    }

    (*env)->CallVoidMethod(env, bundle, codec->jfields.bundle_put_int_id, key, 0);
    if (ff_jni_exception_check(env, 1, codec) < 0) {
        ret = AVERROR_EXTERNAL;
        goto done;
    }
...
typedef struct FFAMediaCodecNdk {
    ...
    // Available since API level 26.
    media_status_t (*setParameters)(AMediaCodec *, AMediaFormat *);
} FFAMediaCodecNdk;
...
{
    FFAMediaCodecNdk *codec = (FFAMediaCodecNdk *)ctx;
    AMediaFormat *params = NULL;
    media_status_t status;

    if (!codec->setParameters) {
        av_log(codec, AV_LOG_DEBUG, "AMediaCodec_setParameters unavailable\n");
        return AVERROR(ENOSYS);
    }
    ...
    status = codec->setParameters(codec->impl, params);
    AMediaFormat_delete(params);

    if (status != AMEDIA_OK) {
        av_log(codec, AV_LOG_WARNING, "requestKeyFrame failed, %d\n", status);
        return AVERROR_EXTERNAL;
    }
